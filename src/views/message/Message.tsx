...
const Message = () => {
	...
	const handleDeleteClick = useCallback(async (id: string) => {
		...
		const [{ delete_log }, { find_logs }] = await _fetch([
			{ delete_log: id },
			{ find_logs: {} },
		])
		...
		delete_log.success
			? (noticebar({ status: 'success', message: '删除成功' }),
			  setLogs(find_logs.data))
			: noticebar({ status: 'error', message: delete_log.errmsg })
	}, [])
		...
		<>
			...
			<Table
				...
				deleteSelectRows={(rows) =>
					handleDeleteSelectionClick(rows.map((row) => row.original._id))
				}
			...
			>
				<div>
					...
					<LogDetail lable="对象" value={Reflect.get(currentRow, 'for_who')} />
					<LogDetail lable="事件" value={Reflect.get(currentRow, 'event')} />
					<LogDetail
						lable="结果"
						value={Reflect.get(currentRow, 'state') ? '成功' : '失败'}
					/>
					<LogDetail lable="信息" value={Reflect.get(currentRow, 'message')} />
...
const DisplayUpdateContent = memo(
	({ beforeUpdate, afterUpdate }: DisplayUpdateContentProps) => {
